...
public class CollectionsDemo {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*singleton:
		 * Signature - <T> Set<T> singleton(T o)
		 * It Returns an immutable set containing only the specified object.
		 * Other methods are also there based on singleton like -  
											  					<T> List<T> singletonList(T o) 
											  					<K,V> Map<K,V> singletonMap(K key, V value)
		 * */
		list.removeAll(Collections.singleton("John"));
		System.out.println("List after removing an element using singleton pattern: " + list); // Output: [Anita, Raj]
		
		/* unmodifiableCollection:
		 * It returns an unmodifiable view of the collection.
		 * Useful when we want to give only read-only access to other or clients who are consuming the data.
		 * */
		...
		System.out.println("unmodifiable: " + unmodifiable); // Output: [Anita, Raj]
		System.out.println("Is Anita there?: " + unmodifiable.contains("Anita")); // Output: true
		/* Below line will throw an exception if we uncomment as the collection is unmodifiable.*/
		//unmodifiable.add("John");
		list.add("John");
		System.out.println("unmodifiable 2: " + unmodifiable); // Output: [Anita, Raj, John]
