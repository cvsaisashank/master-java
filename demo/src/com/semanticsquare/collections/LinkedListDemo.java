...
/* Notes
 * By default Java implements double linked list. Meaning each element has two links - one to the previous element and one to the next element.
 * Advantages:
 * 1.It will allow us to perform operations on both first and last elements of a list.
 * 2. If we want to look for an element at a particular index, then we would have to traverse thru only half of the list. It will do that by checking
 * if the index is close to first or last element and based on that it will start traversing. Head will allow us to navigate either from start or from end.
 * When should we use this?
 * 1.frequent add/remove during iteration. As there are no element shifts happening like in the case of an ArrayList when we add or remove.
 * 2.Better for removeAll() & retainAll()
 * 3.Better for addition and removing elements. add(i, e) and remove(i) method linked list is better than the ArrayList as it completes the operations in O(n/2) as only half are processed. 
 * Random notes:
 * 1. get(i) method is better in ArrayList than LinkedList as ArrayList has fast random access.
 * 2. LinkedList is a doubly-linked list implementation of List & Deque interfaces
 * 3. LinkedList performs both LIFO and FIFO operations in O(1) but it is recommended to use Array Deque.
 * 4. Code below is just copied from ArrayList and change the implementation to LinkedList and everything works just fine as
 *  all the method names are common but there will be a change in implementation.
 *  5. More memory is required in linked list than array list as it needs to hold 3 values like next memory address, last memory address and value.
 *  6. ArrayList is used for retrieving data and LinkedList is used mostly for adding and removing data. 
 * */
public class LinkedListDemo {	
	...
	private static void iteratorDemo(List<Integer> list1) {
		...
		while (iterator.hasNext()) {
			...
			if (element == 9) {
				/*
				 *  we don't get the ConcurrentModificationException exception in iterator when we call iterator method.
				 *  We must invoke the remove method only after `next()` method. No two consecutive remove() methods is possible.
				 *  Because it will remove the item the next() method initiates.
				 * */
				iterator.remove(); 
				/*
				 * forEachRemaining - forEach is defined by iterable. But forEachRemaining is defined by Iterator interface.
				 * Other than element 9, it will invoke the method add in Filter class using method reference syntax(::) for all the elements other than 9.
				 * */
				iterator.forEachRemaining(FilterClass::add);	
			}
		}
		 ...
		 * */
		list1.forEach(FilterClass::filter);
		/*
		 * Within forEach of ArrayList, if we pass an instance of a class, that class shd implement Consumer interface which has an 
		 * accept method or else it will throw an error.
		 * */
		list1.forEach(new FilterClass());	// requires implementing Consumer	
...
class FilterClass implements Consumer {
	static void filter(Integer i) {
		if (i == 1)
			System.out.println("To demo forEach(Filter::filter) " +  i);
	}
	// accept method is Consumer interface implementation.
	public void accept(Object i) {		
		if ((int)i == 1)
			System.out.println("To demo forEach(new Filter) implementation of Consumer " +  i);
	}
		
	static void add(Integer i) {
		System.out.println("to demo forEachRemaining() method " +  i + 7);
	}
