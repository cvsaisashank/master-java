 ...
 * targets.
 * 
 * Not all the methods are there in the below demo like poll, etc.
 * */
public class NavigableTreeSetDemo {
	...
	private static void navigableTreeSetDemo() {
		...
		headSet.add(4);
		/* As we using headSet, the range is, anything less than 74 is acceptable. So adding value 75 throws IllegalArgumentException.*/
		//headSet.add(75); // throws IllegalArgumentException
		System.out.println("set: " + set);// output: [4, 5, 6, 23, 74, 89]
		
		/* subSet(from, to) where from is inclusive and `to` is exclusive*/
		SortedSet<Integer> subSet = set.subSet(5, 74);
		System.out.println("subSet: " + subSet);// output: [5, 6, 23]
		/* Adding add(2) will throw IllegalArgumentException because it is not within the subSet range of 5 and 74*/
		...
		set.add(25);
		System.out.println("subSet after changing original set: " + subSet);// output: [5, 6, 23, 25]
	}
