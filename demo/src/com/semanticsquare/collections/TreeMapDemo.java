...
public class TreeMapDemo {
	  ...
	  private static void treeMapDemo() {
		...
		/*
		 * Q) Why we cannot call setValue() method on the result of similar methods like floorEntry()?
		 * A) Because such methods return an entry but the value that is produced is a snapshot  at that instance of time. So that entry is 
		 * not backed by the original Map. For this reason we can not add or set any value like below. 
		 * If done it throws an `UnsupportedOperationException`. Uncomment the below line to see the exception.
		 * setValue(22) is supported when looping thru entries but when standalone like below.
		 * */
