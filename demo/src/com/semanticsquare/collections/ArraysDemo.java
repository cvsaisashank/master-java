...
class IntUnaryOperatorImpl implements IntUnaryOperator {
	...
	/*Adds 5 to each element in the array in a single statement in one go which is better than a for loop*/
	@Override
	public int applyAsInt(int i) {
		if (iArray != null) {
			return iArray[i]+5; // add 5 to each element in the array
		} else {
			return i; // returns index value of the element in the array.
		}		
	}
}