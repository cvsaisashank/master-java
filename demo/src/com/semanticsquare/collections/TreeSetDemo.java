...
/* Notes
 * Q) WHat is TreeSet
 * A) It is red-black tree implementation of NavigableSet interface. Internally it uses a TreeMap. TreeMap stores key, value pairs. But 
 * TreeSet has only elements. So it will store the elements as keys and while it will store an empty object(new Object()) as the value.
 * - Elements are unique and sorted and unique because it is a set.
 * It is slightly slower than a HashSet but still offers fast look up - O(log(n))
 * */
...
class TitleComparator implements Comparator {
	 ...
	 * A) We can pass our own comparator logic like TitleComparator(), AuthorComparator(), etc. 
	 ...
	 * A) The actual algorithm that is being passed at run time is called as Strategy Design pattern. In this case, the sorting strategy 
